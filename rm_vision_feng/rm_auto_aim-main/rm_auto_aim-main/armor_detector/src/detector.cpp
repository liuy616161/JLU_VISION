...
std::vector<Armor> Detector::matchLights(const std::vector<Light> & lights)
{
  ...
  this->filtered_armors_.clear(); // HACK:
...
void Detector::drawResults(cv::Mat & img)
{
  ...
  // Draw armors
  for (const auto & armor : filtered_armors_) {
    cv::line(img, armor.left_light.top, armor.right_light.bottom, cv::Scalar(255, 100, 100), 2);
    cv::line(img, armor.left_light.bottom, armor.right_light.top, cv::Scalar(255, 100, 100), 2);
  }
