...
void NumberClassifier::extractNumbers(const cv::Mat & src, std::vector<Armor> & armors)
{
  ...
  for (auto & armor : armors) {
    ...
    // HACK: RFID module's LED light will be captured into ROI
    // to avoid interfering with classifier, fill the last 2 rows of pixels black
...
void NumberClassifier::classify(std::vector<Armor> & armors)
{
  ...
  // std::cout<<"armor's number confidence:" << confidence <<std::endl;
  ...
  armors.erase(
      ...
      [this](const Armor & armor) {
        ...
        for (const auto & ignore_class : ignore_classes_) {
          if (armor.number == ignore_class) {
            // std::cout<<"ignore is not good enough"  <<std::endl ;     
            return true;
          }
        }
    ...
    armors.end());
    // if(armors.empty()){
    //   // std::cout<<"numbers is not good enough"  <<std::endl ;     
    // }
}
