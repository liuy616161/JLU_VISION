...
void PredictPitchXY::GimbalControlTransform(float xw, float yw, float zw, float vxw, float vyw, float vzw, float v_yaw,
                                            float r1, float r2, float dz,  float axw,float ayw, int armors_num, float yaw, float* aim_x, float* aim_y,
                                            float* aim_z,float *fire,float robo_yaw,float v)
{
  ...
  float respond_time = 100;   //可以通过打小陀螺测试得到
  ...
  if(fabsf(v_yaw) < 1.8){
    if(v > 0.08){
      ...
      pre_aim[0].x =xw  + vxw * timeDelay + 0.5 * axw * timeDelay * timeDelay ;
      pre_aim[0].y =yw  + vyw * timeDelay + 0.5 * ayw * timeDelay * timeDelay ;
      
      pre_aim[0].yaw = tar_yaw + v_yaw * timeDelay;
    }else{
      pre_aim[0].x =xw - r1 * cos(tar_yaw) + vxw * timeDelay + 0.5 * axw * timeDelay * timeDelay ;
      pre_aim[0].y =yw - r1 * sin(tar_yaw) + vyw * timeDelay + 0.5 * ayw * timeDelay * timeDelay ;
      pre_aim[0].z =zw + vzw * timeDelay;
      pre_aim[0].yaw = tar_yaw + v_yaw * timeDelay;
    }
  }else{
  ...
  if (armors_num == 2)
  {
    for (i = 0; i < 2; i++)
    {
      ...
      // vx = v_yaw * r * sin(tar_position[i].yaw);
      // vy = -v_yaw * r * cos(tar_position[i].yaw);
      // pre_aim[i].x =tar_position[i].x + vxw * timeDelay + vx * timeDelay;
      // pre_aim[i].y =tar_position[i].y + vyw * timeDelay + vy * timeDelay;
      float pre_tmp_yaw = tmp_yaw + v_yaw * timeDelay;
      pre_aim[i].x =xw + vxw * timeDelay +  0.5 * axw * timeDelay * timeDelay - r * cos(pre_tmp_yaw);
      pre_aim[i].y =yw + vyw * timeDelay +  0.5 * ayw * timeDelay * timeDelay - r * sin(pre_tmp_yaw);
