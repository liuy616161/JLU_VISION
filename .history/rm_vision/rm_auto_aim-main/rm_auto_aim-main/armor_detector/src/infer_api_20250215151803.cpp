    ...
    for (int anchor_idx = 0; anchor_idx < num_anchors; anchor_idx++)
    {
        ...
        if (box_prob >= BBOX_CONF_THRESH)
        {
          ...
          switch (box_color) {
            ...
            std::cout<<"point 1:"<<obj.landmarks[0]<<","<<obj.landmarks[1]<<std::endl;
            std::cout<<"point 2:"<<obj.landmarks[2]<<","<<obj.landmarks[3]<<std::endl;
            std::cout<<"point 3:"<<obj.landmarks[4]<<","<<obj.landmarks[5]<<std::endl;
            std::cout<<"point 4:"<<obj.landmarks[6]<<","<<obj.landmarks[7]<<std::endl;
            ...
            points.push_back(cv::Point2f(obj.landmarks[2], obj.landmarks[1]));
...
void OpenvinoInfer::drawResults(cv::Mat &img){
    for(const auto &tmp_armor : tmp_objects){
        for(int i=0;i<4;i++){
            ...
            cv::putText(
                img,std::to_string(i+1),cv::Point2f(tmp_armor.landmarks[i*2],tmp_armor.landmarks[i*2+1]),
