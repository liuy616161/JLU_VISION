...
std::vector<Armor> OpenvinoInfer::infer(cv::Mat &img, int detect_color){
    objects.clear();
    tmp_objects.clear();
    ious.clear();
    ...
    //图像变换
    //缩放并填充边界
    raw_size=cv::Size(img.cols,img.rows);
    auto k=float(640.0 / std::max(img.cols,img.rows));
    int new_width=int(round(img.cols*k));
    int new_height=int(round(img.rows*k));
    ...
    //图像反变换
    img=img(cv::Rect(0,0,new_width,new_height));
    cv::resize(img,img,raw_size,0,0,cv::INTER_LINEAR);
