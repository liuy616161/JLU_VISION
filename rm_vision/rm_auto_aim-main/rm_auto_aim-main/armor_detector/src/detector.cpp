...
cv::Mat Detector::preprocessImage(const cv::Mat & rgb_img)
{
  ...
  // 使用饱和运算处理差异图像
  cv::Mat diff_img;
  if (detect_color == 0) { // 红色
    cv::Mat green_blue;
    cv::addWeighted(channels[1], 0.5, channels[2], 0.5, 0, green_blue);
    cv::subtract(channels[0], green_blue, diff_img, cv::noArray(), CV_8U);
  } else if (detect_color == 1) { // 蓝色
    cv::Mat red_green;
    cv::addWeighted(channels[0], 0.5, channels[1], 0.5, 0, red_green);
    cv::subtract(channels[2], red_green, diff_img, cv::noArray(), CV_8U);
  }
  
  // 应用阈值处理
  cv::Mat binary_img;
  cv::threshold(diff_img, binary_img, binary_thres, 255, cv::THRESH_BINARY);
  */
  cv::Mat gray_img;
...
ArmorType Detector::isArmor(const Light & light_1, const Light & light_2)
{
  ...
  armor_data.center_y = (light_1.center.y + light_2.center.y) / 2;
