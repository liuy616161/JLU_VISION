...
void PredictPitchXY::GimbalControlTransform(float xw, float yw, float zw, float vxw, float vyw, float vzw, float v_yaw,
                                            ...
                                            float* aim_z,float *fire,float robo_yaw,float v,int latency_flag)
{
  //距离偏置修正，一般不修，主要是角度问题
  float static_x = 0.00;
  float static_z = 0.00;
...
<<<<<<< HEAD:rm_vision/rm_auto_aim-main/rm_auto_aim-main/armor_tracker/src/trajectory.cpp
  ...
  if(v_yaw > 4){
      ...
      respond_time =80 ;//////new 5's latency
  }
  if(v_yaw<-4){
    respond_time=100;
=======
...
>>>>>>> 75f713b (clean):rm_vision/rm_auto_aim/armor_tracker/src/trajectory.cpp
  ...
  else if(abs(v) > 0.5 && latency_flag == 2)
  {
    respond_time += 150;
  }
...
//火控策略
    *fire = 0;
    ...
    if(yaw_diff_min < 0.001 && fabsf(v_yaw) > 1.8){
