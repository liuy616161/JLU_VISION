...
<<<<<<<< HEAD:rm_vision/rm_serial_driver-main/rm_serial_driver-main/src/rm_serial_driver.cpp
...
========
    ...
    gain_pub_ = this->create_publisher<std_msgs::msg::Int64>("/gain",10);
    aim_gn_ = this->declare_parameter("gain_aim", 8.0);
    buff_gn_ = this->declare_parameter("gain_buff", 3.0);


   
>>>>>>>> 75f713b (clean):rm_vision/rm_serial_driver-main/src/rm_serial_driver.cpp
...
========
          ...
          if(packet.task_mode == 0) {  // 自瞄模式
              current_exposure_time_ = aim_et_;

              current_gain_ = aim_gn_;
          }else  {  // 能量机关模式   
              current_exposure_time_ = buff_et_;

              current_gain_ = buff_gn_;
          }

          

          // 只有当曝光时间改变时才发布消息
          if(current_exposure_time_ != previous_exposure_time_) {
              exposure_time_msg.data = current_exposure_time_;
              exposure_time_pub_->publish(exposure_time_msg);
              
              // 更新前一个曝光时间
              previous_exposure_time_ = current_exposure_time_;
>>>>>>>> 75f713b (clean):rm_vision/rm_serial_driver-main/src/rm_serial_driver.cpp
          }
           ...
           // 只有当增益改变时才发布消息
          if(current_gain_ != previous_gain_) {
              gain_msg.data = current_gain_;
...
<<<<<<<< HEAD:rm_vision/rm_serial_driver-main/rm_serial_driver-main/src/rm_serial_driver.cpp
    packet.aim_x=msg->armor_img_x;
    packet.aim_y=msg->armor_img_y;
========

    packet.exposure_time = current_exposure_time_/100;
