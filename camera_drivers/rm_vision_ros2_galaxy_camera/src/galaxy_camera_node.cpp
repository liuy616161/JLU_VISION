...
public:
  explicit GalaxyCameraNode(const rclcpp::NodeOptions & options) : Node("galaxy_camera", options)
  {
    ...
    while (true) {
      ...
      status = GXOpenDevice(&stOpenParam, &camera_handle_);
    ...
    bool use_sensor_data_qos = this->declare_parameter<bool>("use_sensor_data_qos");
    ...
    image_msg_.header.frame_id = this->declare_parameter<std::string>("frame_id");
